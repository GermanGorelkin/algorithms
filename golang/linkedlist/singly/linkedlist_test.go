...
func TestNewList(t *testing.T) {
	...
	t.Run("head and counting", func(t *testing.T) {
		if l.head != nil || l.count != 0 {
			t.Errorf("Got head=%v counting=%d expected head=nil counting=0", l.head, l.count)
		}
	})
...
func TestList_AddHead(t *testing.T) {
	...
	l.AddHead(1)
	t.Run("counting with one value", func(t *testing.T) {
		expected := 1
		got := l.count
		if got != expected {
	...
	l.AddHead(2)
	l.AddHead(3)
	t.Run("counting with 3 values", func(t *testing.T) {
		expected := 3
		got := l.count
		if got != expected {
			t.Errorf("got %d want %d", got, expected)
...
func TestList_RemoveLoop(t *testing.T) {
	t.Run("no loop", func(t *testing.T) {
		l := NewList()
		l.AddTail(1)
		l.AddTail(2)
